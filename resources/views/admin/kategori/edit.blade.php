...
<div x-data="{ isSubmitting: false }" class="space-y-6">
    ...
    <div class="space-y-1">
        ...
        <nav class="flex items-center gap-2 text-sm text-slate-500">
            ...
            <svg class="w-4 h-4 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-blue-700 font-semibold">Edit Kategori</span>
        </nav>

        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Kategori</h2>
            <p class="text-sm text-slate-500 mt-1">Perbarui nama kategori berita existing</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/70 shadow-sm max-w-xl">
        <form action="{{ route('proses-edit-kategori', $kategoris->id) }}" 
              method="POST" 
