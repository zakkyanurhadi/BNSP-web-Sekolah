...
<div x-data="{
    ...
    <div class="space-y-1">
        ...
        <nav class="flex items-center gap-2 text-sm text-slate-500">
            ...
            <svg class="w-4 h-4 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-blue-700 font-semibold">Tambah Berita</span>
        </nav>

        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Tambah Berita</h2>
            <p class="text-sm text-slate-500 mt-1">Buat berita baru untuk portal Panel CMS Konten Sekolah</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/70 shadow-sm">
        <form action="{{ route('proses-tambah-berita') }}" 
              method="POST" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="status_berita" 
                            required
                            class="block w-full px-4 py-2.5 text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                        <option value="regular" {{ old('status_berita') === 'regular' ? 'selected' : '' }}>Regular</option>
                        <option value="headline" {{ old('status_berita') === 'headline' ? 'selected' : '' }}>Headline</option>
                    </select>
                    @error('status_berita')
                        <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea id="deskripsi"
                          ...
                          required
                          class="block w-full px-4 py-3 text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit" 
                    ...
                    <span x-text="isSubmitting ? 'Menyimpan...' : 'Simpan Berita'"></span>
                </button>
            </div>
        </form>
    </div>
